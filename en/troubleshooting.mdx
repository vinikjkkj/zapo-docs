...
<Accordion title="QR or 8-digit code — which should I use?" icon="key">
  Both work. QR is the default (`auth_qr` event). For an 8-digit code, call `client.auth.requestPairingCode(phone)` after the `auth_pairing_required` event. See [Authentication](/en/concepts/authentication).
</Accordion>
