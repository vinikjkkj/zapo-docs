...
<Steps>
  ...
  <Step title="Create the store and client">
    The [store](/en/concepts/stores) persists auth and Signal state. Here we use SQLite, writing to `.auth/state.sqlite`.
  ...
  <Step title="Handle pairing">
    ...
    <Tip>
      Prefer an **8-digit pairing code** instead of a QR? See [Authentication](/en/concepts/authentication#pairing-with-a-code).
    </Tip>
  </Step>
