...
A handful of helpers are exported from the package root so you can call them outside any coordinator:
...
For typed business hours (`WaBusinessHoursDay` / `WaBusinessHoursMode` and the frozen `WA_BUSINESS_HOURS_DAYS` / `WA_BUSINESS_HOURS_MODES` constants), see [Profile, privacy & business](/en/guides/profile-privacy#typed-business-hours).
