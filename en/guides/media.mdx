---
title: "Media"
description: "Send images, video, audio, documents, and stickers — and download incoming media."
icon: "image"
---
...
<Warning>
  **Sending media requires [`@zapo-js/media-utils`](/en/installation#sending-media).** The outbound pipeline uses it to probe and process media before upload. Install it and wire the processor through the [`media`](#media-processing) client option whenever your app sends media.
</Warning>
...
client.on('message', async (event) => {
  ...
  // ⚠️ Buffers the entire file in memory — only for small payloads
...
Sending media requires a **media processor**. Install `@zapo-js/media-utils` and pass one through the `media` client option — it probes and processes media (dimensions, duration, thumbnails, waveforms, voice-note normalization) before upload:
