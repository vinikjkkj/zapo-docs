...
Per-chat settings — mute, pin, archive, mark read, delete — live on `client.chat` ([`WaAppStateMutationCoordinator`](/en/reference/client#chat)), because they sync across your devices via app-state:
...
These changes are also surfaced as [`mutation`](/en/concepts/events#state-history--mex) events when made from another device.
