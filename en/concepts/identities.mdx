...
description: "How WhatsApp's phone-number JIDs (PN) and privacy LIDs differ, when each appears in stanzas, and how zapo maps between them when sending and receiving."
icon: "fingerprint"
---
...
A `WaIncomingMessageEvent` carries both identifiers when the server provides them:

| Field | Meaning |
| --- | --- |
| `senderJid` | The sender's **primary** JID for this message (PN or LID, matching the addressing mode). |
| `senderAlt` | The **alternate** form — if `senderJid` is a PN, this is the LID, and vice versa (when available). |
| `recipientJid` | Your receiving JID. |
| `recipientAlt` | The alternate form of the recipient (when available). |

```ts
client.on('message', (event) => {
  console.log('primary:', event.senderJid)   // e.g. 1999...@lid in a LID group
  console.log('alt:    ', event.senderAlt)    // e.g. 5511...@s.whatsapp.net
})
```

So in a LID-addressed group you'll typically see `senderJid` as a `@lid` and `senderAlt` as the phone JID (if the server shares it).
...
<Warning>
  **Always prefer sending by LID when you have one.** The LID is the privacy-preserving, forward-compatible identity WhatsApp is migrating to — addressing a peer by LID is the future-proof choice and avoids leaking/relying on phone numbers. Fall back to the PN only when no LID is available.
  ...
  Get the LID from the incoming event's `senderAlt` (when `senderJid` is a PN) or resolve it with [`getLidsByPhoneNumbers`](#mapping-a-phone-number-to-its-lid).
</Warning>

<Tip>
  **In a group, always reply to `event.chatJid`** (the group JID), not to a participant's JID. For 1:1 chats, prefer the peer's LID; `event.chatJid` / `event.senderJid` also work whether they are a PN or a LID.
</Tip>
