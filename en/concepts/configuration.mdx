...
<ParamField path="addons" type="{ autoDecrypt?: boolean }">
  Set `autoDecrypt: true` to automatically decrypt encrypted addons (poll votes, reactions, …) and emit them as typed [`message_addon`](/en/guides/receiving-messages#addons) events. Otherwise call `client.message.tryDecryptAddon(event)` yourself.
</ParamField>
