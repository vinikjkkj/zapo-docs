...
<Note>
  For the [MCP server](/en/dev-tools#mcp-server) the same overrides are available as `MCP_DEVICE_OS_DISPLAY` / `MCP_DEVICE_OS_VERSION` environment variables — when only the version is set the display name is still derived from the host, so pin both to keep the advertised pair consistent.
</Note>
...
<ParamField path="history" type="WaHistorySyncOptions">
  ...
  - `groupBundles?: boolean` — opt into downloading the group-history bundle a member may share after somebody joins a group; emits [`group_history_bundle`](/en/concepts/events#state-history--mex). **Off by default** — a bundle is media a third party pushes at this account unprompted, so fetching it is opt-in. Bundles addressed to other members are dropped either way. See [Groups → sharing group history](/en/guides/groups#sharing-group-history).
