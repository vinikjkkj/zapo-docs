...
As soon as `backends` contains at least one entry, **every persistence domain must be assigned explicitly** in `providers` (`auth`, `signal`, `preKey`, `session`, `identity`, `senderKey`, `appState`, `privacyToken`, `messages`, `threads`, `contacts`). Both the TypeScript types and a runtime check enforce this — `createStore` throws and lists the missing `providers.*` keys when any are omitted.
...
This guard exists because partial coverage is almost always a footgun: persisting only `auth` while letting Signal state, app-state, or the mailbox silently fall back to memory means the device pairs once and then loses its protocol state on every restart. Pick `'memory'` deliberately if that is what you want.
