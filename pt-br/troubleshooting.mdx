...
<Accordion title="QR ou código de 8 dígitos — qual usar?" icon="key">
  Os dois funcionam. O QR é o padrão (evento `auth_qr`). Para um código de 8 dígitos, chame `client.auth.requestPairingCode(phone)` após o evento `auth_pairing_required`. Veja [Autenticação](/pt-br/concepts/authentication).
</Accordion>
...
<CardGroup cols={2}>
  <Card title="Arquitetura em detalhes" icon="layer-group" href="/pt-br/concepts/internals">
    Entenda as camadas para depurar no nível do protocolo.
  </Card>
  <Card title="API low-level" icon="terminal" href="/pt-br/reference/low-level">
