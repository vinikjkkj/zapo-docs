...
- [**Fake server**](#fake-server) (`@zapo-js/fake-server`) — um servidor WhatsApp falso in-process usado para benchmarks e cross-check tests da lib. Para os testes E2E **do seu próprio app**, veja o guia [Testes end-to-end](/pt-br/guides/e2e-testing).
...
O `@zapo-js/fake-server` é o servidor WhatsApp Web falso in-process que a lib usa para testes cross-check e benchmarks. Também é o caminho recomendado para rodar **testes end-to-end do seu próprio app** sem tocar no WhatsApp — veja o guia dedicado [Testes end-to-end](/pt-br/guides/e2e-testing), que cobre install, API programática, fixtures `FakePeer`, isolamento multi-sessão e recipes de CI.

O resto desta seção é limitado ao uso interno da lib — benchmarking.
