...
Assim que `backends` contém ao menos uma entrada, **todo domínio de persistência precisa ser atribuído explicitamente** em `providers` (`auth`, `signal`, `preKey`, `session`, `identity`, `senderKey`, `appState`, `privacyToken`, `messages`, `threads`, `contacts`). Tanto os tipos TypeScript quanto uma verificação em tempo de execução garantem isso — `createStore` lança um erro listando as chaves `providers.*` faltantes quando alguma é omitida.
...
A verificação existe porque cobertura parcial é quase sempre uma armadilha: persistir apenas `auth` enquanto o estado do Signal, o app-state ou a caixa de mensagens silenciosamente recaem para a memória significa que o dispositivo pareia uma vez e depois perde o estado de protocolo a cada reinício. Escolha `'memory'` de forma deliberada se for esse o seu objetivo.
