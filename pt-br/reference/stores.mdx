...
O tipo do backend é `WaStoreBackend<S, C>` — os dois parâmetros assumem por padrão a matriz completa de domínios (todo domínio de persistência mais todo domínio de cache). Todo pacote `@zapo-js/store-*` incluído entrega um backend completo; um backend parcial escrito à mão se estreita com `WaStoreBackend<S, C>` e o `createStore()` recusa domínios não declarados em tempo de compilação. Veja [Backends customizados](/pt-br/concepts/stores#custom-backends).
...
| `cacheTtlMs` | `{ retryMs?, groupMetadataMs?, chatMetadataMs?, deviceListMs?, messageSecretMs? }` | TTLs de cache. |
...
### TTL por domínio do store {#per-domain-store-ttl}
...
Os cinco domínios de cache (`retry`, `groupMetadata`, `chatMetadata`, `deviceList`, `messageSecret`) usam um TTL definido em `cacheTtlMs`. A forma como entradas expiradas são *removidas* varia por backend:
