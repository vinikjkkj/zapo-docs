...
| `remoteJid` | `string \| null` | Attr `from` bruto — JID do grupo para grupos, JID PN ou LID device para 1:1. |
| `fromMe` | `boolean` | Já resolvido contra o `meJid` da conta. |
| `id` | `string \| undefined` | Id da stanza. |
| `participant` | `string \| null` | Attr `participant` bruto; `null` para stanzas que não são de grupo. |

<Note>
  O predicate vê os attrs `from` / `participant` **brutos** — sem resolução de alt-attr PN ↔ LID. O formato descriptor é quem cuida disso automaticamente; se você precisa de matching por JID com cobertura de forma alt *junto* de uma regra custom, prefira o `remoteJid` do descriptor com `only`.
</Note>
