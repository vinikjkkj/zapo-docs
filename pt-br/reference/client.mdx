...
description: "Referência completa de métodos do WaClient e de cada coordinator, fundamentada no código-fonte."
icon: "code"
---
...
`WaGroupCoordinator` — veja [Grupos & comunidades](/pt-br/guides/groups). Métodos que retornam `BinaryNode` dão o resultado bruto do IQ; operações sobre participantes codificam resultados **por participante** como filhos `<participant jid error>` (o IQ tem sucesso mesmo que alguns falhem).
...
| `setSetting` | `(groupJid, setting, enabled) => Promise<void>` | Alterna uma flag booleana do grupo (`announce`, `restrict`, `ephemeral`, `group_history`, `allow_admin_reports`, `no_frequently_forwarded`, flags de comunidade). |
