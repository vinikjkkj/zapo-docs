...
Payloads concretos de `client.message.send(jid, …)` para cada tipo bruto vivem no [guia de envios proto brutos](/pt-br/guides/raw-sends). Esta referência traz apenas as tabelas de mapeamento de tipos acima; o guia tem os exemplos de envio mais os cards de pagamento (PIX / review-and-pay).

### Wrapper de mensagem temporária (ephemeralMessage) {#disappearing-wrapper-ephemeralmessage}

Envolva qualquer mensagem para que ela herde o timer ephemeral do chat. Isto é uma mecânica de wrapper — a mensagem interna é o que efetivamente renderiza:
...
Para o toggle ephemeral do chat inteiro (`protocolMessage` + `EPHEMERAL_SETTING`), veja a seção [Alternar mensagens temporárias](/pt-br/guides/raw-sends#toggle-disappearing-messages-ephemeral-setting) no guia.
