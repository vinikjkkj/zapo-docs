...
| `disableGroupEphemeralAutoInject` | `boolean` | Pula a injeção automática do ephemeral-setting ao enviar para um grupo com disappearing-mode ligado. Redundante quando `expirationSeconds` está definido. |
...
<Tip>
  Para enviar uma única mensagem **sem** expiração para um grupo com disappearing-mode ligado, prefira `disableGroupEphemeralAutoInject: true` em vez de `expirationSeconds: 0` — o último ainda grava `expiration=0` no `contextInfo` de saída.
</Tip>
