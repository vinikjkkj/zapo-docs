...
As configurações por chat — silenciar, fixar, arquivar, marcar como lido, excluir — ficam em `client.chat` ([`WaAppStateMutationCoordinator`](/pt-br/reference/client#chat)), porque sincronizam entre seus dispositivos via app-state:
...
await client.chat.clearChat(chatJid)
await client.chat.deleteChat(chatJid)
```

Essas mudanças também são expostas como eventos [`mutation`](/pt-br/concepts/events#state-history--mex) quando feitas a partir de outro dispositivo.
