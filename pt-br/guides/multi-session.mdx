...
- **`await client.logout()`** — remoção lógica. Apaga o estado persistente daquele `sessionId` (sujeito a [`logoutStoreClear`](/pt-br/concepts/configuration#logoutStoreClear)) e desvincula o device no servidor. O bundle continua no map interno da store até você destruí-lo ou o processo terminar; um `WaClient({ store, sessionId })` posterior com o mesmo id reutiliza o mesmo bundle.
