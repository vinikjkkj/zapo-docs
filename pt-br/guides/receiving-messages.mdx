...
| --- | --- | --- |
| `key.remoteJid` | `string` | O JID da conversa (grupo ou 1:1). |
...
<Note>
  Eventos `receipt` ainda expõem `stanzaId` / `chatJid` diretamente (eles estendem `WaIncomingBaseEvent`); a renomeação se aplica somente aos payloads de `message`, `message_addon` e `message_bot_chunk`, que agora usam `event.key`.
</Note>
