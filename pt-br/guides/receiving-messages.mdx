...
| `key.senderUsername` | `string?` | Handle do autor, sem o `@`. **Ausente em stanzas 1:1 auto-enviadas** — o handle do peer aparece em `key.recipientUsername` no lugar. |
| `key.recipientUsername` | `string?` | Handle do peer, sem o `@`. Preenchido em stanzas 1:1 auto-enviadas (onde `senderUsername` é ausente) e em qualquer mensagem que carregue `recipient_username` / `peer_recipient_username`. |
...
## Recuperando mensagens indisponíveis {#recovering-unavailable-messages}

Algumas stanzas `<message>` chegam como placeholder `<unavailable/>` em vez de corpo criptografado — uma view-once consumida, uma mensagem hosted ou bot que o servidor não conseguiu fazer fanout, ou um placeholder de fanout simples cujo primário ainda tem o plaintext. As quatro variantes aparecem no evento [`message_unavailable`](/pt-br/concepts/events#unavailable-messages) com discriminador `kind`.

Placeholders de fanout simples (`kind === 'other'`) são **recuperáveis**: a lib enfileira uma peer request `PLACEHOLDER_MESSAGE_RESEND` ao device primário, e o payload recuperado chega depois como evento `message` normal, com o mesmo `key.id`. Os consumidores sabem que precisam esperar por ele via `resendRequested`:

```ts
client.on('message_unavailable', (event) => {
  if (event.resendRequested) {
    console.log('waiting for primary to resend', event.key.id)
    // Um evento `message` com o mesmo key.id virá em seguida (best-effort)
  }
})
```

A recuperação é best-effort — a request enfileirada não é persistida, então uma peer message que falha não é retentada. `resendRequested` é `false` para os kinds não recuperáveis (`'view_once'`, `'hosted'`, `'bot'`), para stanzas além da janela de idade do servidor (dirigida pela AB prop `placeholder_message_resend_maximum_days_limit`, não pelos 30 dias hardcoded de antes) e em sessões mobile-primary (não há outro device para pedir). A lib **não** dá timeout no resend pendente — um primário de verdade pode responder além do padrão de 30s de peer-request; o wa-web se comporta do mesmo jeito.
...
O `WaIncomingReceiptEvent` também carrega `participantUsername?: string` — o handle do participante sem o `@`, lido do atributo `participant_username` da stanza quando presente.
