...
<Note>
  Batches só sobem enquanto o client está conectado **e** a conta terminou de registrar (`credentials.meJid` populado). Uma batch produzida offline, ou durante a janela pré-login em um pareamento novo, é dropada no flush — o plugin loga um trace e segue em frente. Uma vez que a sessão está totalmente logada, batches futuras sobem normalmente.
</Note>
