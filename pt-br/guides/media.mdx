---
title: "Mídia"
description: "Envie imagens, vídeo, áudio, documentos e figurinhas — e baixe mídias recebidas."
icon: "image"
---
...
<Warning>
  **Enviar mídia requer [`@zapo-js/media-utils`](/pt-br/installation#sending-media).** O pipeline de saída o usa para sondar e processar a mídia antes do upload. Instale-o e conecte o processador através da opção de client [`media`](#media-processing) sempre que seu app enviar mídia.
</Warning>
...
client.on('message', async (event) => {
  ...
  // ⚠️ Bufferiza o arquivo inteiro em memória — apenas para payloads pequenos
...
Enviar mídia requer um **processador de mídia**. Instale `@zapo-js/media-utils` e passe um através da opção de client `media` — ele sonda e processa a mídia (dimensões, duração, miniaturas, waveforms, normalização de notas de voz) antes do upload:
